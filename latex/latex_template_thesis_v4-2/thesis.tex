...
The act of archiving and transcribing parliamentary meetings is not new. Most big governmental organisations such as the European Parlemen \footnote{https://www.europarl.europa.eu/plenary/nl/debates-video.html} and the Dutch House of Representatives \footnote{https://debatdirect.tweedekamer.nl/} host an online archive that offers an elementary search mechanism. 
...
Many vector databases allow for a combination between ANN powered vector similarity search and probabilistic searches. TODO
...
\chapter{Methods}
\section{Crawler}
Before starting any analysis work, the first sub research question \textit{what are the different archive formats local authorities host in order to comply with the Woo and how are these formats exploited to retrieve as much information as possible?} needs to be answered.
...
In the current analysis pipeline code, two Whisper models have been implemented: one client using MLX, a Python array framework allowing fast machine learning on Apple silicon devices, and one using PyTorch. 
...
\subsection{Agenda points}\label{sec:agendapoints}
For the research question \textit{how accurately can such a video be segmented into the meaningfully different parts which are discussed}, a agenda point topic segmentation client needs to be written. It should take in the transcript of a meeting, and it should return a list of agenda points and times, specifying when these topics start and end.
...
The concept of topic segmentation is quite simple. First, the transcript of a meeting is fed into an LLM with the task to label the most important topics discussed during the meeting, in chronological order. 
...
The created Weaviate client allows the user to easily perform default operations such as creating and deleting collections (similar to tables in relational databases), inserting and deleting entries and getting collection metadata. 
...
To make all previously mentioned functionalities accessible to the end user, I developed a user-friendly web application using front-end framework Vue3.
...
\section{Video analysis}\label{sec:vidanalysisExp}
Answering the research question \textit{what state-of-the-art video- and audio analysis tools can be used to improve searchability of the meetings, and how accurate are these in the given circumstances}? involves evaluating the two utilized analysis tools in the created analysis pipeline: Whisper and pyannote.
...
To evaluate Whisper's performance on the scraped meetings, I manually transcribed 40 minutes of meetings, selected randomly from seven different meetings. To account for structural- and unimportant transcription differences, a normalisation is applied to both the reference and the hypothesis text. 
...
One interesting observation is the large difference between Zoom meetings and regular meetings. For a couple years during the nationwide COVID regulations, the municipality meetings took place online. During these meetings, every participant used their own device and microphone, a general lower audio quality, with large quality differences between different speakers probably contributing to a lower transcription accuracy.
...
To test how well pyannote performs on the municipality dataset, I manually annotated 30 random minutes of meetings, distributed over five different meetings. I used audio tool Audacity \footnote{https://www.audacityteam.org/} to obtain millisecond-accurate timestamps. During manual diarisation, speakers were named arbitrarily. After diarisation, the manual speaker names were mapped to the speaker names given by pyannote to make sure the confusion rate metric can be performed.
...
\begin{figure}
    \centering
    \includegraphics[width=0.89\textwidth]{images/error_rates_box_plots.png}
...
One potential reason as to why pyannote seems to perform better on these meetings, is the relative simple structure of the meetings. There is not too much overlapping speech and speakers generally are speaking through good microphones.

\subsection{Agenda points}
Using the agenda points provided by some meetings as a ground truth, I tested the video segmentation script described in section \ref{sec:agendapoints}, answering the research question \textit{how accurately can such a video be segmented into the meaningfully different parts which are discussed}?

I ran the script on 10 different meetings, using their provided agenda points as a ground truth. Next, I counted the number of missed agenda points and the number of false agenda points.

\begin{figure}
    \centering
    \includegraphics[width=0.85\textwidth]{images/topicSegmentation.png}
    \caption{Agenda point segmentation results}
    \label{fig:agendaSegmResults}
\end{figure}

The disappointing results from figure \ref{fig:agendaSegmResults} indicate that the developed method of automatic topic segmentation in these types of government meetings is not sufficient yet. 
...
To Answer the main research question \textit{how can AI be utilized in order to increase information retrievability of large video archives, in particular of democratically elected councils}? and the subquestion \textit{how well can the developed search system be integrated with a large language model, creating a helpful chat bot capable of answering questions and providing complementary information when needed}?, I gathered 10 participants and tasked them to find the answer to five questions using five different search methods.
...
For each participant, the search methods were randomly mapped to a question. The participants had three minutes to answer each question. In the scenario where a satisfactory answer is not found within the given time, the search is aborted and this is noted.
...
\begin{figure}
    \centering
    \includegraphics[width=0.90\textwidth]{images/search_methods_box_plot.png}
    \caption{search engine results}
    \label{fig:engineResults}
\end{figure}

Figure \ref{fig:engineResults} shows the results of this experiment, disregarding the search attempts where an answer was not found in time. Observing these results, it can be concluded that the contemporary method of scrolling through the video is by far the least efficient. On top of the fact that the average search time is almost three times as slow as the second slowest method, in 70\% of the cases, a valid answer was not found within the given three minutes.
