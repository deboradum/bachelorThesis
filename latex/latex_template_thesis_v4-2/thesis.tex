...
% Title Page
\title{Improving government transparency with a searchable video archive}
...
In the end I chose sentence-transformers/all-mpnet-base-v2. Its small size, its small size of only 109 million and multilingual capabilities make it the perfect model for this task.

\subsection{Agenda points}\label{sec:agendapoints}
Some of the meetings already have agenda points provided by the archiving host. For the meetings where this is not the case, a custom agenda point generator that utilizes LLMs has been written.

First, the transcript of a meeting is fed into an LLM with the task to give the most important subjects of the meeting. 
Next, for each subject, the LLM is given the task to figure out when the agenda point starts in the meeting. This is done using a sliding window over the transcript. A couple sentences at a time, along with their starting time, are added to the input, after which the LLM will return either true or false depending on if the agenda point starts at the correct time.

\section{Retrieval}
\subsection{Weaviate}\label{sec:weaviate}
Unlike the aforementioned mentioned clients, the Weaviate client is not as modular. One of the reasons for this is that releases of new and better database systems are less common that new models. 
...
One interesting observation is the large difference between Zoom meetings and regular meetings. For a couple years during the nationwide COVID regulations, the municipality meetings took place online. During these meetings, every participant used their own device and microphone, a general lower audio quality, with large quality differences between different speakers.
...
% TODO Mention that the good/ better results miught be explained by the general format of the meetings: There is not much overlapping speech and proffesional microphones are used. (except for the zoom meeings, where there is more overlapping speech due to lag and worse microphones)
...
a
% TODO: grafieken en data parsen en hierin zetten!

\chapter{Conclusion}
Looking back at the main research question \textit{how can AI be utilized in order to increase information retrievability of large video archives, in particular of democratically elected councils}? It can be concluded from figure \ref{} that a strong and user friendly system of information retrievability has been developed, as indicated by the significantly quicker retrieval times. 
...
Finally, a chat bot system with access to the Weaviate database allows the user to ask questions regarding specific meetings in a conversational style. As shown in figure \ref{}, this chat bot scores ....., indicating that .....
